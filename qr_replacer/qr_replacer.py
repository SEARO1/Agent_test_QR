"""
QR Code Replacer Pipeline v0.4.1
===============================
        ...
        try:
            ...
            if retval and points is not None:
                ...
                try:
                    ...
                    pass
                all_results.append({"name": name, "points": points, "decoded": decoded, "variant": variant})
        except Exception:
            pass
    if not all_results:
...
def expand_carrier_quad(pts: np.ndarray, ratio: float, img_h: int, img_w: int) -> np.ndarray:
    """Expand quad outward from center by ratio. DEPRECATED."""
...
def _scan_edge_brightness(img: np.ndarray, start_pt: np.ndarray, direction: np.ndarray, max_px: int = 80, step_px: int = 5) -> dict:
...
def detect_carrier_region(img: np.ndarray, qr_pts: np.ndarray) -> dict:
    ...
    result = {"paper_bgr": paper_bgr.tolist(), "corners": {}, "recommended_expand_px": {}}
    for i, (name, corner) in enumerate(zip(["TL", "TR", "BR", "BL"], qr_pts)):
...
def _detect_content_threats(img: np.ndarray, qr_pts: np.ndarray) -> dict:
    ...
    dir_vectors = [np.array([0, -1]), np.array([0, 1]), np.array([-1, 0]), np.array([1, 0])]
    for name, dvec in zip(dir_names, dir_vectors):
        ...
        if samples:
            ...
            threats[name] = {"variance": float(variance), "dark_ratio": float(dark_ratio),
...
def compute_smart_expansion(qr_pts: np.ndarray, carrier_info: dict, img_h: int, img_w: int, strategy: str = "smart") -> np.ndarray:
    center = qr_pts.mean(axis=0)
    if strategy == "tight":
        dirs = qr_pts - center
        ...
        expanded_pts_list = []
        corner_threats = [
            threats.get("top", {}).get("has_content", False),
            threats.get("top", {}).get("has_content", False),
            threats.get("bottom", {}).get("has_content", False),
            threats.get("bottom", {}).get("has_content", False),
        ]
        for i, (corner, name) in enumerate(zip(qr_pts, ["TL", "TR", "BR", "BL"])):
            dir_norm = norms[i]
...
def generate_qr_image(payload: str, quiet_zone: int = 4, fg_color: tuple = (0, 0, 0), bg_color: tuple = (255, 255, 255)) -> np.ndarray:
    ...
    for v in range(1, 40):
        if payload_len <= _qr_version_capacity(v):
            version = v
            break
    qr = qrcode.QRCode(version=version, error_correction=qrcode.constants.ERROR_CORRECT_M, box_size=10, border=quiet_zone)
    qr.add_data(payload)
    qr.make()
...
def generate_carrier_patch(payload: str, carrier_size: tuple, fg_color: tuple = (0, 0, 0), bg_color: tuple = (255, 255, 255),
                           local_appearance: Optional[dict] = None, add_texture: bool = True, jpeg_simulation: bool = True) -> tuple:
...
def composite_two_stage(original: np.ndarray, qr_patch: np.ndarray, qr_mask: np.ndarray, carrier_mask: np.ndarray,
                        expanded_pts: np.ndarray, feather_px: int = 3, paper_fill_dilation: int = 3) -> np.ndarray:
    """
    Two-stage composite that eliminates dark border artifact.
    Stage 1: Fill carrier region with paper color using dilated feathered mask (erases old QR content).
    Stage 2: Overlay QR modules directly onto the paper-colored base using a re-thresholded QR mask.

    Key: Warp the full carrier patch (QR+papers), then derive QR mask AFTER warp via thresholding.
    """
    h, w = original.shape[:2]

    # Warp full carrier patch (QR embedded in paper) to destination
    warped_qr_patch = perspective_warp(qr_patch, expanded_pts, (h, w))
    warped_carrier_mask = perspective_warp(carrier_mask, expanded_pts, (h, w))

    # Derive QR mask AFTER warp by thresholding the warped grayscale.
    # This avoids interpolation artifacts from warping a binary mask.
    warped_gray = cv2.cvtColor(warped_qr_patch, cv2.COLOR_BGR2GRAY)
    _, warped_qr_mask_thresh = cv2.threshold(warped_gray, 128, 255, cv2.THRESH_BINARY_INV)

    # Dilate carrier mask for paper fill (ensures full old QR erasure)
    dilate_kernel = np.ones((paper_fill_dilation * 2 + 1, paper_fill_dilation * 2 + 1), np.uint8)
    dilated_carrier_mask = cv2.dilate(warped_carrier_mask, dilate_kernel)

    # Sample paper color from warped patch background (non-QR white areas)
    bg_mask = (warped_qr_mask_thresh == 255)
    if bg_mask.sum() > 100:
        paper_color = warped_qr_patch[bg_mask].mean(axis=0).astype(np.uint8)
    else:
        paper_color = np.array([220, 220, 220])

    # Stage 1: Paper fill - blend paper-colored canvas onto original using dilated feathered mask
    sigma = max(1, feather_px)
    blur_ks = sigma * 6 + 1
    fill_mask_f = cv2.GaussianBlur(dilated_carrier_mask.astype(np.float32), (blur_ks, blur_ks), sigma) / 255.0
    stage1_result = original.copy().astype(np.float32)
    for c in range(3):
        stage1_result[:, :, c] = (
            original[:, :, c].astype(np.float32) * (1 - fill_mask_f) +
            float(paper_color[c]) * fill_mask_f
        )
    stage1_result = np.clip(stage1_result, 0, 255).astype(np.uint8)

    # Stage 2: QR overlay - place QR modules directly onto paper-colored base (no feathering on QR)
    qr_overlay = stage1_result.copy()
    for c in range(3):
        qr_overlay[:, :, c] = np.where(
            warped_qr_mask_thresh > 0,
            warped_qr_patch[:, :, c],
            stage1_result[:, :, c]
        )
    return qr_overlay
...
def match_appearance_to_local(warped_patch: np.ndarray, warped_mask: np.ndarray, original_img: np.ndarray,
                              quad: np.ndarray, blur_sigma: float = 0.0) -> np.ndarray:
    ...
    outer_ring = cv2.subtract(cv2.dilate(warped_mask, np.ones((7, 7), np.uint8), iterations=2), warped_mask)
    ...
    if len(outer_pixels) > 0:
        ...
        for c in range(3):
            result[:, :, c] = np.where(patch_mask > 0, np.clip(result[:, :, c] + adjust[c] * 0.6, 0, 255), original[:, :, c])
    if blur_sigma > 0.1:
...
def blend_feather(original: np.ndarray, patch: np.ndarray, mask: np.ndarray, feather_px: int = 3) -> np.ndarray:
    sigma = max(1, feather_px)
    blur_ks = sigma * 6 + 1
    mask_f = cv2.GaussianBlur(mask, (blur_ks, blur_ks), sigma).astype(np.float32) / 255.0
    result = original.copy().astype(np.float32)
    for c in range(3):
        result[:, :, c] = original[:, :, c].astype(np.float32) * (1 - mask_f) + patch[:, :, c].astype(np.float32) * mask_f
    return result.astype(np.uint8)


def blend_seamless(original: np.ndarray, patch: np.ndarray, mask: np.ndarray, quad: np.ndarray) -> np.ndarray:
    ...
    try:
        return cv2.seamlessClone(patch.astype(np.uint8), original, mask, (cx, cy), cv2.MIXED_CLONE)
    except Exception:
        pass
    try:
        return cv2.seamlessClone(patch.astype(np.uint8), original, mask, (cx, cy), cv2.NORMAL_CLONE)
    except Exception:
        pass
...
def _score_candidate(candidate_img: np.ndarray, original_img: np.ndarray, quad: np.ndarray, payload: str, carrier_info: dict) -> dict:
...
def estimate_local_appearance(img: np.ndarray, quad: np.ndarray, ring_width: int = 15) -> dict:
    ...
    if len(ring_pixels) == 0:
        return {"mean_bgr": np.array([220, 220, 220]), "contrast": 20.0, "color_cast": np.array([0, 0, 0]), "sharpness": 1.0}
    ...
    try:
        ...
        sharpness = 1.0
    return {"mean_bgr": mean_bgr, "contrast": contrast, "color_cast": color_cast, "sharpness": float(sharpness)}


# ─── Main pipeline ────────────────────────────────────────────────────────────

def replace_qr_pipeline(input_image_path: str, new_payload: str, output_path: str,
                        debug_dir: Optional[str] = None, mode: str = "single", replace_all: bool = False,
                        blend_mode: str = "feather", carrier_expand_ratio: float = 1.2,
                        edge_feather_px: int = 3, blur_sigma: float = 0.0,
                        jpeg_simulation: bool = True, verify: bool = True,
                        qr_fg: tuple = (0, 0, 0), qr_bg: tuple = (255, 255, 255)) -> QRReplacementResult:
    os.makedirs(debug_dir, exist_ok=True) if debug_dir else None
    img = cv2.imread(input_image_path)
    if img is None:
        return QRReplacementResult(output_image_path=output_path, detected_points=None, old_decoded_text=None,
                                   new_decoded_text=None, success=False, reason_if_failed=f"Cannot load image: {input_image_path}")
    h, w = img.shape[:2]
    ...
    if points is None:
        ...
        return QRReplacementResult(output_image_path=output_path, detected_points=None, old_decoded_text=old_decoded,
                                   new_decoded_text=None, success=False, method_used=None, reason_if_failed="No QR code detected")
    ...
    debug_report["steps"].append({
        ...
        "content_threats": {k: {"has_content": v["has_content"]} for k, v in carrier_info["content_threats"].items()}
    })
    ...
    candidate_strategies = [("tight", "tight"), ("smart", "smart"), ("conservative", "conservative")]
    candidates = []
    for strat_name, strategy in candidate_strategies:
        ...
        carrier_patch, qr_mask, patch_mask = generate_carrier_patch(
            new_payload, carrier_size, fg_color=qr_fg, bg_color=qr_bg,
            local_appearance=local_appearance, add_texture=True, jpeg_simulation=jpeg_simulation)
        warped_carrier_mask = perspective_warp(patch_mask, expanded, (h, w))
        warped_carrier_patch = perspective_warp(carrier_patch, expanded, (h, w))
        if blend_mode == "two_stage":
            blended = composite_two_stage(img, carrier_patch, qr_mask, patch_mask, expanded,
                                           feather_px=edge_feather_px, paper_fill_dilation=3)
        elif blend_mode == "seamless":
            matched = match_appearance_to_local(warped_carrier_patch, warped_carrier_mask, img, expanded, blur_sigma=blur_sigma)
        ...
        total_score = (scores["decode_success"] * 5.0 + scores["brightness_match"] * 1.0 +
                      scores["content_protection"] * 2.0 - 0.1 * (carrier_bb[2] * carrier_bb[3]) / 10000)
        candidates.append({
            "strategy": strat_name, "expanded_pts": expanded.copy(), "carrier_size": carrier_size,
            "blended": blended, "scores": scores, "total_score": total_score,
            "qr_mask": qr_mask, "patch_mask": patch_mask, "carrier_patch": carrier_patch,
        })
        if debug_dir:
            dbg_c = img.copy()
        ...
        debug_report["candidates"].append({
            "strategy": strat_name, "carrier_size": carrier_size,
            "scores": scores, "total_score": total_score, "expand_pts": expanded.tolist(),
        })
    ...
    debug_report["steps"].append({
        "step": "candidate_selected", "strategy": selected_strategy,
        "total_score": best["total_score"], "scores": best["scores"],
    })
    ...
    if not success:
        retry_strats = [
            {"blend_mode": "two_stage", "paper_dilation": 2, "feather_px": max(1, edge_feather_px - 1)},
            {"blend_mode": "two_stage", "paper_dilation": 4, "feather_px": max(1, edge_feather_px - 2)},
            {"blend_mode": "two_stage", "paper_dilation": 5, "feather_px": 2},
            {"blend_mode": "two_stage", "paper_dilation": 3, "feather_px": 1},
        ]
        for strat in retry_strats:
            if not verify:
                break
            retries += 1
            s_blend = strat.get("blend_mode", blend_mode)
            if s_blend == "two_stage":
                retried = composite_two_stage(img, best["carrier_patch"], best["qr_mask"], best["patch_mask"],
                                               expanded_pts, feather_px=strat.get("feather_px", edge_feather_px),
                                               paper_fill_dilation=strat.get("paper_dilation", 3))
            else:
                warped_carrier_mask = perspective_warp(best["patch_mask"], expanded_pts, (h, w))
                warped_carrier_patch = perspective_warp(best["carrier_patch"], expanded_pts, (h, w))
                retried_matched = match_appearance_to_local(warped_carrier_patch, warped_carrier_mask, img, expanded_pts, blur_sigma=blur_sigma)
                if s_blend == "seamless":
                    retried = blend_seamless(img, retried_matched, warped_carrier_mask, expanded_pts)
                else:
                    retried = blend_feather(img, retried_matched, warped_carrier_mask, feather_px=strat.get("feather_px", edge_feather_px))
    ...
    return QRReplacementResult(
        output_image_path=output_path, detected_points=pts_ordered, old_decoded_text=old_decoded,
        new_decoded_text=new_payload if success else None, success=success, method_used=method_used,
        expanded_points=expanded_pts, selected_blend_mode=blend_mode, retries_used=retries,
        reason_if_failed=last_error if not success else None, debug_report=debug_report,
    )
